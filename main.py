...
def generate_feedback_text_wb_before_NY(user_name, prod_val, feedback_text, has_photo):
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "В конце каждого ответа поздравь покупателя с наступающим новым 2025 годом и пожелай чего-нибудь хорошего. Добавь подпись 'Ваш MissYourKiss'"
...
def generate_feedback_text_wb_NY(user_name, prod_val, feedback_text, has_photo):
    client = OpenAI(api_key=os.getenv('OPENAI_API_KEY'))
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    # Имя клиента и завершение ответа
                    "Если имя клиента кажется ненастоящим, не используйте его в ответе. "
                    "В конце каждого ответа поздравь покупателя с новым 2025 годом и пожелай чего-нибудь хорошего. Добавь подпись 'Ваш MissYourKiss'"
...
def process_and_answer_feedbacks():
    ...
    for feedback in feedbacks:
        ...
        if before_ny_end >= today >= datetime.min.date():
            # Если сегодня до 31.12.2024 включительно
            answer = generate_feedback_text_wb_before_NY(
                user_name=user_name,
                prod_val=product_valuation,
                feedback_text=feedback_text,
        ...
        elif ny_start <= today <= ny_end:
            # Если сегодня между 31.12.2024 и 18.01.2025 включительно
            answer = generate_feedback_text_wb_NY(
                user_name=user_name,
                prod_val=product_valuation,
                feedback_text=feedback_text,
                has_photo=bool(photo_links)
